...
describe "async #subscribe" do
  let(:event_class) { OneModule::TestEvent }

  let(:callable) do
    ActiveEventStore::TestEventSubscriber = 
      Module.new do
        class << self
          def events
            @events ||= []
  ...
  let(:event) { event_class.new(user_id: 123, user: { name: "Jones" }) }

  after do
    ActiveEventStore.send(:remove_const, :TestEventSubscriber) if
      ActiveEventStore.const_defined?(:TestEventSubscriber)
  end
  ...
  it "enqueues job" do
    ...
    end.to have_enqueued_job(ActiveEventStore::TestEventSubscriber::SubscriberJob).on_queue("events_subscribers")
  end
  ...
  it "calls subscriber when performed", active_job: :perform do
    ...
    expect(callable.events.last.user_id).to eq 123
  end
end
